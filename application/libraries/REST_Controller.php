...
abstract class REST_Controller extends CI_Controller
{
	...
	*/
	...
	protected function _check_access() 
	{
		...
		$this->rest->db->like('controller', $this->uri->uri_string());
	...
	private function _config_jwt($ths)
	{
		$ths->load->config('jwt');

		$ths->load->library(array('Jwt'));		
		$token = base64_decode($ths->input->get("token"));
		if (count(explode('.',$token)) == 3)
		{
			$decodedToken = JWT::decode($token,$ths->config->item('jwt_key'),array('HS256'));
	        if ($decodedToken != false) {        					        	
	        	$ths->load->config('rest');
	        	$ths->config->set_item('rest_auth',false);	        	
	        	$ths->_args[$ths->config->item('rest_key_name')] = $decodedToken->API_KEY;	        	
	        }
